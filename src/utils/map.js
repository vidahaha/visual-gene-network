...
(function (d, o) {
	...
	w: while (i--) {
		...
		if (e.src) {
			...
			if (a.hostname.match(/\.revolvermaps\.com$/i) && a.pathname.match(/^\/?0\/0\/8\.js$/)) {
				s = o.b.length;
				while (s--)
				...
				function b(i, t, u) {
					...
					if (!('IntersectionObserverEntry' in window && 'isIntersecting' in window.IntersectionObserverEntry.prototype)) i.onload = function () {
						(function l(a) {
							if (!d.hidden) {
					...
					i.setAttribute('src', '//' + a.hostname + '/w/8/a/a2.php' + a.search);
					u = i;
					if (!p('s')) {
